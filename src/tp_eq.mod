...
# Variables que estaban en el modelo original pero ya no se usan
...
s.t. super {h in HORAS, c in CANALES, d in DIAS}: sum{e in EVENTOS} y[e, h, c, d] * (2 - sum{dd in DEPORTES} INTERCALABLE[dd] * DEP_EVENTO[e, dd]) <= 2;
...
s.t. evt_un_p {e in EVENTOS}: sum{p in PERIODISTAS} cubierto[e, p] <= 1;

# Un equipo periodistico cubre a lo sumo un evento
s.t. p_un_evt {p in PERIODISTAS}: sum{e in EVENTOS} cubierto[e, p] <= 1;

# Si un bloque se transmite, debe ser cubierto por algun equipo
s.t. t_cubre {e in EVENTOS}: t[e] <= sum{p in PERIODISTAS} cubierto[e, p];

# Si el evento es cubierto, debe transmitirse en algun canal
s.t. cubre_t {e in EVENTOS}: sum{p in PERIODISTAS} cubierto[e, p] <= t[e];

# Relaciono con la variable que dice si un equipo cubre un evento
#s.t. p_cubre_e_inf {e in EVENTOS, p in PERIODISTAS, d in DIAS}: cubierto[e, p] * DIAS_EVENTOS[e, d] <= sum{h in HORAS} cubierto[e, p];
#s.t. p_cubre_e_sup {e in EVENTOS, p in PERIODISTAS, d in DIAS}: M * cubierto[e, p] * DIAS_EVENTOS[e, d] >= sum{h in HORAS} cubierto[e, p];
